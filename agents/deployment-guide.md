...
meta:
  ...
  description: >-
    USE WHEN any Amplifier Online question arises (Azure Container Apps/Web Apps/Static Web Apps
    deployment): what it is; Platform/Provisioner Service/CLI architecture; stack choice
    (web-app-aca, internal-service-aca, web-app-awa, static-web-app); repo readiness/project
    analysis (Dockerfiles, ACR refs); authoring/validating amplifier-online.yaml + Key Vault
...
7. **CI/CD setup: container workflows need no Azure secrets.** Container/Web-App deploys authenticate
   ...
   (`AO_PROVISIONER_URL`, `AO_PROVISIONER_AUDIENCE`, `AO_PROJECT_NAME`) for you. The only secret is
   `AZURE_STATIC_WEB_APPS_API_TOKEN`, for Static Web App deployments (available after the first `up`).
